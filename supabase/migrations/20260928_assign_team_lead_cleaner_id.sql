-- After assign_team_and_sync_roster (60926): persist team **lead** on bookings.cleaner_id as well as
-- payout_owner_cleaner_id. Many cleaner queries use cleaner_id.eq.<viewer>; null cleaner_id hid deep/move
-- team jobs from the lead dashboard even when payout_owner was correct.
...
comment on function public.assign_team_and_sync_roster(uuid, uuid, uuid, integer, text, text, timestamptz) is
  'Atomically assigns team on booking (lead on cleaner_id + payout_owner_cleaner_id), rebuilds booking_cleaners. JSON: {ok:true,variant} or {ok:false,reason:race_lost}. Admin variant preserves lifecycle columns once accepted or beyond.';

update public.bookings b
set cleaner_id = b.payout_owner_cleaner_id
