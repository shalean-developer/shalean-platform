-- Restore payout_owner_cleaner_id persistence on assign_team_and_sync_roster (regressed in 60855/60883).
-- TS selects a capability-qualified owner; sync_booking_cleaners_for_team_booking reads bookings.payout_owner_cleaner_id
-- for lead role — without this column, lead fell back to min(cleaner_id) among active roster members.
...
comment on function public.assign_team_and_sync_roster(uuid, uuid, uuid, integer, text, text, timestamptz) is
  'Atomically assigns team on booking (persisting payout_owner_cleaner_id), rebuilds booking_cleaners. JSON: {ok:true,variant} or {ok:false,reason:race_lost}. Admin variant preserves cleaner_response_status/en_route_at/started_at/accepted_at once accepted or beyond.';
