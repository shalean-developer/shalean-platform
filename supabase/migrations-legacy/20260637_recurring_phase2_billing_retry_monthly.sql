-- Recurring engine + billing/retry/monthly fields (all idempotent).
-- Duplicates 20260636_recurring_bookings_engine.sql so this file succeeds if 20260636 was skipped
-- or you run migrations out of order (e.g. only this file in the SQL editor). Safe when 20260636 already ran.
...
-- Brownfield: add monthly + booking columns if an older recurring_bookings row existed without them.
alter table public.recurring_bookings
  add column if not exists monthly_pattern text not null default 'mirror_start_date'
    check (monthly_pattern in ('mirror_start_date', 'nth_weekday', 'last_weekday'));

alter table public.recurring_bookings
  add column if not exists monthly_nth smallint null
    check (monthly_nth is null or monthly_nth between 1 and 4);
