-- When a past service date is team-assigned later, roster sync must use the later of
-- visit date and assignment date so cleaners who joined before assignment are included.
...
begin
  ...
  select b.team_id,
    ...
    into b_team, b_date, b_assigned, b_lead, b_is_team, b_finalized
...
comment on function public.sync_booking_cleaners_for_team_booking(uuid, text) is
  'Rebuild booking_cleaners from team_members using greatest(visit date, assigned_at date) for membership window.';
