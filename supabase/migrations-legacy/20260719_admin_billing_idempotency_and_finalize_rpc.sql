-- Cross-instance idempotency for admin billing PATCH + atomic impact read + profile update (single transaction).

-- ---------------------------------------------------------------------------
-- Idempotency replay cache (NO_CHANGE / UPDATED only; 12 min TTL)
-- ---------------------------------------------------------------------------
...
create index if not exists admin_billing_idempotency_user_expires_idx
  on public.admin_billing_idempotency (user_id, expires_at desc);

comment on table public.admin_billing_idempotency is
  'Replay cache for admin customer billing PATCH (Idempotency-Key). Only terminal NO_CHANGE/UPDATED responses are stored.';

alter table public.admin_billing_idempotency enable row level security;

grant select, insert, delete, update on public.admin_billing_idempotency to service_role;

-- ---------------------------------------------------------------------------
-- Atomic finalize: lock profile, compute Johannesburg-month impact, apply guards, UPDATE
-- ---------------------------------------------------------------------------
