-- admin_billing_switch_finalize still counted bookings via legacy user_id; production uses customer_id (20261021).
...
comment on function public.admin_billing_switch_finalize(uuid, text, text, boolean, boolean, boolean, boolean) is
  'Admin billing switch: FOR UPDATE on user_profiles, Johannesburg-month impact (customer_id or legacy user_id), confirmation guards, then UPDATE.';
