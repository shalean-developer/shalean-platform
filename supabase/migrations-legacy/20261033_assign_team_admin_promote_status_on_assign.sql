-- Admin team assignment set cleaner_id (60928) but left status=pending, violating
-- bookings_assigned_requires_status. Promote pending / pending_assignment / offered → assigned
-- when admin assigns a team (matches performAdminAssignTeam locked-roster header patch).
...
comment on function public.assign_team_and_sync_roster(uuid, uuid, uuid, integer, text, text, timestamptz) is
  'Atomically assigns team on booking (lead on cleaner_id + payout_owner_cleaner_id), rebuilds booking_cleaners. JSON: {ok:true,variant} or {ok:false,reason:race_lost}. Admin variant promotes pending/offered to assigned and preserves lifecycle columns once accepted or beyond.';
