...
> **🔎 R2 deep scan (2026-07-17) — root cause confirmed; gate stays NO-GO.** The "Connected to Google but saving the account failed" symptom was fully traced in **`docs/audits/marketing/MKT-001A-PROD-R2-google-business-save-failure.md`** (evidence: `evidence/mkt-001a-prod-r2-google-business-save-failure-2026-07-17T0640Z.json`). **Primary root cause (configuration-only):** the Google **Business Profile APIs are disabled** on the OAuth client's Google Cloud project `525459256770`. The flow succeeds through OAuth token exchange, then fails at **account discovery** (403 → 429) and returns **before** token encryption and the `social_accounts` write — so encryption/persistence are never reached. **Secondary (code, now fixed in PR #41):** the callback collapsed every post-exchange failure into an opaque `save_failed` and the UI discarded the reason; **PR #41** (`fix/mkt-001a-google-business-save-failure` → `staging`) adds a sanitized error classifier + actionable Connected-Accounts message (no change to discovery/encryption/persistence behaviour; typecheck / lint / test:critical 134 / test 3445 / migrations-validate / webpack build all green). **Required operator actions before GO:** (1) enable `mybusinessaccountmanagement`, `mybusinessbusinessinformation`, and `mybusiness` APIs in GCP `525459256770`; (2) wait for propagation; (3) avoid rapid reconnect retries (so 429 does not mask the result); (4) re-scope `MARKETING_OAUTH_ENCRYPTION_KEY` to **Production-only** (per §3.2.1); (5) re-run the staging OAuth smoke once and verify `oauth_token_exchange_ok → list_accounts_ok → list_locations_ok → save_connection_ok`; (6) confirm a `google_business` row exists with `access_token` (and `refresh_token` when present) beginning `v2:` (record format/row-count only, never values). Production remains untouched.
...
- [ ] §3.1 operator smoke green on staging (or formally deferred + rationale recorded) — ⛔ **BLOCKED:** OAuth Re-encryption not verified. R1 fixed the redirect-URI (token exchange succeeds); **R2** (`MKT-001A-PROD-R2-google-business-save-failure.md`) confirms the connection save fails at **Google account discovery** because the **Business Profile APIs are disabled** on GCP project `525459256770` (config-only). Diagnosability fix in **PR #41**. SSRF / Idempotency / Connected-UI passed.
...
**Current decision: NO-GO** (prerequisites outstanding). The active blocker is the **§3.1 OAuth Re-encryption smoke** — now root-caused by **R2** to **disabled Google Business Profile APIs** on GCP project `525459256770` (config-only; tracked in `MKT-001A-PROD-R2-google-business-save-failure.md`, with the diagnosability fix in **PR #41 → `staging`**); the §3.2 production key scope remediation (`MARKETING_OAUTH_ENCRYPTION_KEY` → Production-only) is also still open. Once the operator enables the APIs, re-runs the staging smoke to `save_connection_ok`, verifies a `v2:` `google_business` row, and re-scopes the key, this gate is re-assessed. On a clean GO execution + §7 PASS, **MKT-001A closes** and MKT-001B is unblocked. Do **not** open the `staging → main` release PR yet.
