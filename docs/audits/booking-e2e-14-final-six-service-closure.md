...
## Office lifecycle — SHL-BK-000048

After preferred-offer acceptance, the Office booking advanced correctly:

- accepted_at persisted,
- en_route_at persisted,
- started_at persisted,
- booking status: `in_progress`,
- dispatch status: `assigned`,
- cleaner response status: `started`,
- payout owner / cleaner ownership: Test Cleaner A,
- cleaner earning display: R250,
- persisted solo payout: R250,
- internal earnings: R250,
- payout status remains pending with no batch/link/transfer,
- Office maps to the standard payout policy by design.

Cleaner job details also show the correct Office title, date/time, address, 4h duration, 1 bathroom, small office size, and R250 job earning.

B14-010 UI note: cleaner job details expose technical booking-line-item names generated by the authoritative backfill layer, including `office-cleaning (authoritative subtotal backfill)`, `Service fee (backfill)`, and `Bathrooms (scope)`. Lifecycle/payment/payout are unaffected, but these labels should be normalized before final release polish.
