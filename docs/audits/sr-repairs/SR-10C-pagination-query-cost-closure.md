...
Audit base: `integration/shalean-release` at `0bfa9b41b1a37fbc4a4de979238370d3091c4b14`
...
The active page loader reuses the merged SR-10A saved-address enrichment helper. That helper collects missing-address candidates from the returned page and performs paged batched `customer_saved_addresses` lookups using `.in("user_id", ownerIds)` and `.in("suburb", suburbs)`, exact case-sensitive suburb keys, and deterministic `(created_at DESC, id ASC)` ordering.
