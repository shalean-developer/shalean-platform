...
type Props = {
  ...
  right?: ReactNode;
  /** Hide chevron when not pressable or custom right */
  showChevron?: boolean;
  danger?: boolean;
  ...
  const content = (
    <View className="min-h-touch flex-row items-center gap-3 px-4 py-3">
      ...
      <View className="flex-1">
        <Text className={`text-base font-medium ${danger ? "text-danger" : "text-ink"}`}>
          {label}
        </Text>
        {value ? <Text className="mt-0.5 text-sm text-ink-muted">{value}</Text> : null}
      </View>
      {right}
      {chevron ? (
