...
export function ErrorState({ title, message, onRetry }: Props) {
  return (
    <View className="flex-1 items-center justify-center px-6 py-10" accessibilityRole="alert">
      ...
      {message ? <Text className="mb-5 text-center text-sm leading-5 text-ink-muted">{message}</Text> : null}
...
export function EmptyState({
  ...
  return (
    <View className="flex-1 items-center justify-center px-6 py-16" accessibilityRole="text">
      ...
      <Text className="mb-2 text-center text-lg font-semibold text-ink">{title}</Text>
      {message ? <Text className="text-center text-sm leading-5 text-ink-muted">{message}</Text> : null}
    </View>
  );
}
