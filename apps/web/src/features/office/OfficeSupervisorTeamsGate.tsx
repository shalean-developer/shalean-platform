...
export function OfficeSupervisorTeamsGate({ children }: { children: ReactNode }) {
  ...
  const canViewEarnings = data.can_view_earnings === true;
  ...
  return (
    <div className="space-y-5" data-supervisor-my-team>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border bg-white p-4 shadow-sm"><p className="text-xs font-semibold uppercase text-slate-500">Team earnings this month</p><p className="mt-1 text-2xl font-bold text-slate-900">{canViewEarnings && earnings ? money(earnings.month_total_cents) : "Restricted"}</p></div>
        <div className="rounded-2xl border bg-white p-4 shadow-sm"><p className="text-xs font-semibold uppercase text-slate-500">Paid earnings</p><p className="mt-1 text-2xl font-bold text-slate-900">{canViewEarnings && earnings ? money(earnings.paid_cents) : "Restricted"}</p>{canViewEarnings && earnings ? <p className="mt-1 text-xs text-slate-500">Approved {money(earnings.approved_cents)} · Processing {money(earnings.processing_cents)} · Pending {money(earnings.pending_cents)}</p> : null}</div>
      </div>
