...
export function buildFunnelNarrativeSummary(input: FunnelNarrativeInput): string {
  ...
  if (input.funnelStartSessions <= 0 && checkoutReached <= 0 && paid > 0) {
    parts.push(
      `${paid} paid booking(s) recorded with limited service-stage funnel events — step drop-off charts may under-report until booking_events views accumulate.`,
    );
  } else if (checkoutReached > 0 && paid > 0 && paid < checkoutReached) {
    const paidPct = Math.round((paid / checkoutReached) * 1000) / 10;
    parts.push(
      `${checkoutReached} sessions reached checkout; ${paid} completed payment (${paidPct}%). Service to checkout reach is ${input.conversionRatePct.toFixed(1)}% across ${input.funnelStartSessions} starters.`,
    );
  } else {
    parts.push(
      `Roughly ${input.conversionRatePct.toFixed(1)}% of service sessions reached checkout, across ${input.funnelStartSessions} starters in this window.`,
    );
  }
