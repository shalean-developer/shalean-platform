...
export async function customerCareCaseWorkItems(): Promise<OfficeWorkItem[]> {
  ...
  const { data, error } = await admin
    ...
    .select("id,case_number,category,priority,status,customer_name,booking_id,opened_at,first_response_due_at,resolution_due_at,resolved_at,closed_at")
    .is("closed_at", null)
  ...
  return ((data ?? []) as CaseRow[]).flatMap((row) => {
    ...
    return [{
      ...
      title: overdue ? `${row.case_number} is overdue` : `${row.case_number} needs Customer Care action`,
