...
describe("Owner Command Centre access and KPI presentation", () => {
  ...
  it("keeps quick actions permission-scoped", () => {
    ...
    expect(supervisorActions.some((action) => action.href.includes("security"))).toBe(false);
  ...
  it("does not change permission-scoped navigation destinations for owner quick actions", () => {
    ...
    expect(policyForOfficePath("/office/booking-profitability")?.anyOf).toEqual(["profit.view"]);
    expect(policyForOfficePath("/office/ops-health")?.anyOf).toEqual(["ops.health.view"]);
  });
});
