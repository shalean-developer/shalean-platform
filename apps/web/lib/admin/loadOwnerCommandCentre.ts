...
async function loadEligiblePayoutCents(admin: SupabaseClient): Promise<number> {
  const { data, error } = await admin
    .from("bookings")
    .select("cleaner_id, payout_owner_cleaner_id, payout_frozen_cents, display_earnings_cents, cleaner_earnings_total_cents, cleaner_payout_cents")
    .eq("payout_status", "eligible");
  if (error) throw new Error(error.message);
  let total = 0;
  for (const row of data ?? []) {
    total += resolveCleanerEarningsCents({
...
async function loadSecurityCounts(admin: SupabaseClient): Promise<{ criticalAuditAlerts: number; recentPermissionChanges: number }> {
...
async function loadTodayBookings(admin: SupabaseClient, today: string) {
  const { data, error } = await admin.from("bookings").select(SCHEDULE_BOOKING_SELECT).eq("date", today).order("time", { ascending: true }).limit(5000);
  if (error) throw new Error(error.message);
  return data ?? [];
}
...
async function loadOpsExceptionCount(admin: SupabaseClient): Promise<number> {
  const { data, error } = await admin
    ...
    .limit(5000);
  if (error) throw new Error(error.message);
  const snap = computeOpsSnapshotFromRows((data ?? []) as OpsSnapshotRow[]);
...
export async function loadOwnerCommandCentre(admin: SupabaseClient, viewerUserId: string, now = new Date()): Promise<OwnerCommandCentrePayload> {
  ...
  const revenuePromise = fetchAdminDashboardRevenueSummary(admin, now)
    ...
    .catch((error: unknown) => {
      sectionErrors.businessHealth = error instanceof Error ? error.message : "Failed to load dashboard revenue.";
    });
  ...
  const financePromise = loadFinancialDashboard(admin, monthRange.from, monthRange.to)
    .then((finance) => {
      ...
      sections.cashFlow.outstandingCustomerPaymentsCents = finance.executive_kpis.outstanding_customer_payments_cents;
      ...
      if (finance.untrusted_incomplete_team.booking_count > 0 && finance.profit.customer_revenue_cents === 0 && finance.untrusted_incomplete_team.customer_revenue_cents > 0) {
    ...
    .catch((error: unknown) => {
      sectionErrors.businessHealth = sectionErrors.businessHealth ?? (error instanceof Error ? error.message : "Failed to load financial dashboard.");
      sectionErrors.cashFlow = error instanceof Error ? error.message : "Failed to load financial dashboard.";
      sectionErrors.summaries = error instanceof Error ? error.message : "Failed to load financial summary.";
    });
  ...
  const cashPromise = Promise.all([
    ...
    sections.cashFlow.netCashPositionCents = cash.summary.cash_in_bank_cents + cash.summary.petty_cash_cents;
  }).catch((error: unknown) => {
    sectionErrors.cashFlow = sectionErrors.cashFlow ?? (error instanceof Error ? error.message : "Failed to load cash flow.");
  });

  const payoutPromise = Promise.all([
    listMoneyActionProposals(admin, { status: "pending", limit: 100, offset: 0, viewerUserId }),
    ...
    for (const item of proposals.items) {
      if (item.proposed_total_cents != null && Number.isFinite(item.proposed_total_cents)) pendingAmount += item.proposed_total_cents;
    ...
    sectionErrors.payoutApprovals = error instanceof Error ? error.message : "Failed to load payout approvals.";
  });

  const securityPromise = loadSecurityCounts(admin).then((security) => {
  ...
  const todayPromise = Promise.all([
    ...
    sections.todaySnapshot.estimatedGrossProfitCents = stats.completed > 0 && dayReport.totals.visit_count === 0 ? null : dayReport.totals.company_earnings_cents;
    sections.todaySnapshot.lateOrExceptionCount = exceptionCount;
    sections.businessHealth.completedBookingsToday = stats.completed;
  }).catch((error: unknown) => {
    sectionErrors.todaySnapshot = error instanceof Error ? error.message : "Failed to load today's snapshot.";
  });
  ...
  const summariesPromise = (async () => {
    ...
    const [bookingsRes, priorRes, cleanersRes, appsRes] = await Promise.all([
      ...
      admin.from("bookings").select("customer_id, payment_status, payment_completed_at").eq("payment_status", "success").not("payment_completed_at", "is", null).not("customer_id", "is", null).lt("payment_completed_at", priorEndIso).limit(20000),
    ...
    const analytics = computeOfficeAnalyticsSummary((bookingsRes.data ?? []) as OfficeAnalyticsBookingRow[], extractPriorCustomerIds(priorRes.data ?? []), now, window);
    ...
    sections.summaries.bookingServices = analytics.servicePopularity.slice(0, 8).map((row) => ({ label: row.name, count: row.count, revenueZar: null }));
    ...
    sectionErrors.summaries = sectionErrors.summaries ?? (error instanceof Error ? error.message : "Failed to load owner summaries.");
  });

  await Promise.all([revenuePromise, financePromise, cashPromise, payoutPromise, securityPromise, todayPromise, summariesPromise]);
  return { ok: true, generatedAt: now.toISOString(), sources: OWNER_COMMAND_CENTRE_SOURCES, ...sections, sectionErrors };
}
