...
function isMissingRpcError(
  ...
  return new RegExp(
    `(?:could not find the function|function .*${functionName}.* does not exist)`,
    "i",
  ).test(error.message ?? "");
}

function serviceRowsFromPairs(
  pairs: ServicePair[],
): OwnerCommandCentreAnalyticsRollup["bookingServices"] {
...
async function loadPriorCustomerIds(
