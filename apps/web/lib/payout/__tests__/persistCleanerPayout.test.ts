...
describe("persistCleanerPayoutIfUnset", { timeout: 60_000 }, () => {
  ...
  it("blocks team payout persistence when payout_owner_cleaner_id is missing", async () => {
    const teamBookingId = "bbbbbbbb-bbbb-4bbb-bbbb-bbbbbbbbbbb1";
    const teamId = "aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaa1";
    const cLead = "11111111-1111-4111-8111-111111111111";
    const cMem2 = "22222222-2222-4222-8222-222222222222";
    ...
    expect(result).toEqual({ ok: false, error: "Team job missing payout_owner_cleaner_id" });
  ...
  it("blocks team payout persistence when payout owner does not match the roster lead", async () => {
    const teamBookingId = "bbbbbbbb-bbbb-4bbb-bbbb-bbbbbbbbbbb2";
    const teamId = "aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaa2";
    const cLead = "11111111-1111-4111-8111-111111111111";
    const cMem2 = "22222222-2222-4222-8222-222222222222";
    ...
    const { persistCleanerPayoutIfUnset } = await import("@/lib/payout/persistCleanerPayout");
    const result = await persistCleanerPayoutIfUnset({ admin: admin as unknown as never, bookingId: teamBookingId, cleanerId: cMem2 });

    expect(result).toEqual({ ok: false, error: "Team payout owner does not match booking roster lead" });
    expect(admin.updateCount.bookings ?? 0).toBe(0);
    expect(admin.insertCount.team_job_member_payouts ?? 0).toBe(0);
    expect(admin.tables.team_job_member_payouts).toHaveLength(0);
  });
  ...
  it("blocks team payout persistence when payout owner is not active on the team", async () => {
    ...
    expect(result).toEqual({ ok: false, error: "Team payout owner is not an active team member" });
    expect(admin.updateCount.bookings ?? 0).toBe(0);
    expect(admin.insertCount.team_job_member_payouts ?? 0).toBe(0);
    expect(admin.tables.team_job_member_payouts).toHaveLength(0);
  });
