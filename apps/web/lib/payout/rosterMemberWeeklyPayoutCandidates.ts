...
export type RosterMemberWeeklyPayoutCandidate = {
  id: string;
  booking_id: string;
  payout_cents: number;
  bonus_cents: number;
...
export async function listRosterMemberWeeklyPayoutCandidates(params: {
  admin: SupabaseClient;
  cleanerId: string;
  periodStart: string;
  periodEnd: string;
  invoiceStatusById: Map<string, string>;
}): Promise<RosterMemberWeeklyPayoutCandidate[]> {
  const { admin, cleanerId, periodStart, periodEnd, invoiceStatusById } = params;

  const { data: memberRows, error: memberErr } = await admin
    .from("booking_roster_member_payouts")
    .select("id, booking_id, payout_cents, bonus_cents")
  ...
  const out: RosterMemberWeeklyPayoutCandidate[] = [];
  for (const raw of memberRows) {
    ...
    const bonusCents = Math.max(0, Math.floor(Number(member.bonus_cents) || 0));
    if (payoutCents + bonusCents <= 0) continue;
...
export function rosterMemberWeeklyPayoutTotalCents(rows: readonly RosterMemberWeeklyPayoutCandidate[]): number {
  return rows.reduce((sum, row) => sum + row.payout_cents + row.bonus_cents, 0);
}
