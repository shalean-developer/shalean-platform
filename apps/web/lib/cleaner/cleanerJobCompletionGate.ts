...
export type CleanerJobCompletionGateResult =
  | CleanerJobCompletionGateSuccess
  | CleanerJobCompletionGateFailure;
...
export function evaluateCleanerJobCompletionGate(
  ...
  if (durationMinutes == null) {
    return {
      ok: false,
      code: "missing_persisted_duration",
      error: "Job duration is not on file yet. Contact support before completing.",
      blockedCodes: ["missing_persisted_duration"],
      durationMinutes: null,
    };
  }

  const summary =
    isStructuredPricingBreakdown(row.pricing_summary) ? row.pricing_summary : null;
  if (summary) {
    ...
    if (typeof sig !== "string" || !sig.trim()) {
      return {
        ...
        error: "Job quote is not verified. Contact support before completing.",
        blockedCodes: ["quote_signature_missing"],
        durationMinutes,
      };
    }
  }
  ...
  if (elapsedMinutes == null) {
    return {
      ok: false,
      code: "minimum_duration_not_elapsed",
      error: "Start the job before completing so on-site time can be recorded.",
      blockedCodes: ["minimum_duration_not_elapsed"],
      durationMinutes,
      remainingMinutes: Math.ceil(durationMinutes * CLEANER_COMPLETION_MIN_ELAPSED_RATIO),
      requiredMinutes: Math.ceil(durationMinutes * CLEANER_COMPLETION_MIN_ELAPSED_RATIO),
    };
  }
  ...
  if (elapsedMinutes + 1e-6 < requiredMinutes) {
    ...
    return {
      ok: false,
      code: "minimum_duration_not_elapsed",
      error: `Allow at least ${Math.ceil(requiredMinutes)} minutes on site before completing (${remainingMinutes} min remaining).`,
      blockedCodes: ["minimum_duration_not_elapsed"],
      durationMinutes,
      elapsedMinutes,
      requiredMinutes,
...
function elapsedMinutesFromStartedAtRow(
...
export function buildAdminCompletionGateOverridePatch(params: {
  ...
  return {
    admin_completion_gate_override_at: new Date().toISOString(),
    admin_completion_gate_override_by:
      (typeof params.adminEmail === "string" && params.adminEmail.trim()) || params.adminUserId || null,
    admin_completion_gate_override_reason: reason || null,
    admin_completion_gate_override_codes: params.blockedCodes,
  };
}

/** When admin completes despite cleaner gate failures, persist audit columns on the booking row. */
export function mergeAdminCompletionGateOverrideUpdates(
  updates: Record<string, unknown>,
  params: {
    beforeRow: CleanerJobCompletionGateRow;
    adminEmail?: string | null;
    adminUserId?: string | null;
    reason: string;
    nowMs?: number;
  ...
  if (gate.ok) {
    return { gate, overrideApplied: false };
  }
  Object.assign(
    updates,
