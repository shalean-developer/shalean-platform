...
function weightedScore(components: Array<{score:number|null;weight:number}>, penalty:number) {
  const available = components.filter((c)=>c.score != null);
  const availableWeight = available.reduce((s,c)=>s+c.weight,0);
  const totalWeight = components.reduce((s,c)=>s+c.weight,0);
  if (!availableWeight) return { overall:null as number|null, coverage:0 };
  const base = available.reduce((s,c)=>s+(c.score as number)*c.weight,0) / availableWeight;
  return { overall:clamp(base-penalty), coverage:clamp((availableWeight/totalWeight)*100) };
}
function scheduledStartIso(date:string|null,time:string|null):number|null {
  const d=String(date??"").trim(); const t=String(time??"").trim().slice(0,5);
...
export async function loadCleanerPerformanceScorecards(
  ...
  input?: { cleanerId?: string | null; cleanerIds?: string[] | null; days?: number },
  ...
  const toDate=new Date(); const fromDate=new Date(toDate.getTime()-days*86_400_000);
  const toIso=toDate.toISOString(); const fromIso=fromDate.toISOString();
  ...
  if (input?.cleanerId) cleanerQuery=cleanerQuery.eq("id",input.cleanerId);
    ...
    const ids=[...new Set(input.cleanerIds.map((id)=>String(id).trim()).filter(Boolean))];
    if (!ids.length) return { scorecards:[], from:fromIso, to:toIso };
    cleanerQuery=cleanerQuery.in("id",ids);
  }
  ...
  const {data:rosterRows,error:rosterError}=await admin.from("booking_cleaners")
    .select("booking_id, cleaner_id, completed_at, assigned_at").in("cleaner_id",cleanerIds);
  if (rosterError) throw new Error(rosterError.message);
  const roster=(rosterRows??[]) as Array<{booking_id:string;cleaner_id:string;completed_at:string|null;assigned_at:string|null}>;
  const allBookingIds=[...new Set(roster.map((r)=>r.booking_id))];
  const bookings:Array<{id:string;status:string|null;date:string|null;time:string|null;started_at:string|null;completed_at:string|null;is_test:boolean|null}>=[];
  for (let i=0;i<allBookingIds.length;i+=300) {
    const ids=allBookingIds.slice(i,i+300);
    const {data,error}=await admin.from("bookings").select("id, status, date, time, started_at, completed_at, is_test")
      .in("id",ids).gte("date",fromYmd).lte("date",toYmd);
  ...
  const cards=cleaners.map((cleaner):CleanerPerformanceScorecard=>{
    ...
    let qualityRelatedCases=0,openQualityCases=0;
    for (const c of cases) { if (!c.booking_id||!bookingToCleaners.get(c.booking_id)?.has(cleaner.id)) continue; if (!QUALITY_CASE_CATEGORIES.has(String(c.category??"").trim().toLowerCase())) continue; qualityRelatedCases++; if (!["resolved","closed"].includes(String(c.status??"").toLowerCase())) openQualityCases++; }
