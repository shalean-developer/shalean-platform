  ...
  return (
    <div
      className={cn(
        ...
        emphasize
          ? "border-emerald-200 bg-gradient-to-br from-emerald-600 to-emerald-800 text-white shadow-emerald-900/20"
    ...
    >
      ...
      <p className={cn("mt-2 text-sm font-medium leading-snug sm:text-base", emphasize ? "text-emerald-50" : "text-zinc-600")}>
        {label}
      </p>
      {hint ? (
        <p className={cn("mt-2 text-xs", emphasize ? "text-emerald-200/90" : "text-zinc-500")}>{hint}</p>
      ) : null}
    </div>
  );
}
