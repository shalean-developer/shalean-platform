...
function FacebookIcon({ className }: { className?: string }) {
  return <svg viewBox="0 0 24 24" className={className} fill="currentColor" aria-hidden><path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.99 3.66 9.13 8.44 9.88V14.89h-2.54V12h2.54V9.8c0-2.51 1.49-3.89 3.77-3.89 1.09 0 2.23.19 2.23.19v2.46h-1.26c-1.24 0-1.63.77-1.63 1.56V12h2.78l-.44 2.89h-2.34v6.99C18.34 21.13 22 16.99 22 12z" /></svg>;
}
...
export function SiteFooter({ stackFloats = false, showFloatingWhatsApp = true }: { stackFloats?: boolean; showFloatingWhatsApp?: boolean }) {
  return (
    <>
        ...
        <PublicPageContainer>
          <div className="grid gap-8 sm:grid-cols-2 sm:gap-10 lg:grid-cols-5">
            <div className="lg:col-span-1">
              <div className="inline-flex rounded-lg bg-white px-2.5 py-1.5 shadow-sm"><ShaleanNavLogo className="h-8 w-auto sm:h-9" intrinsicHeight={80} /></div>
              <p className="mt-4 max-w-xs text-sm leading-relaxed text-slate-400">Reliable, professional cleaning services in Cape Town. We clean so you can live.</p>
              <div className="mt-5 flex items-center gap-3">
                {SHALEAN_SOCIAL_LINKS.map((link) => {
                  ...
                  return <a key={link.id} href={link.href} target="_blank" rel="noopener noreferrer" aria-label={`Shalean on ${link.label}`} onClick={link.id === "whatsapp" ? () => trackEvent("whatsapp_click") : undefined} className="flex h-11 w-11 items-center justify-center rounded-full bg-white/10 text-slate-300 transition hover:bg-white/20 hover:text-white sm:h-10 sm:w-10"><Icon className="h-4 w-4" /></a>;
                })}
              </div>
            </div>
          ...
          <div className="mt-12 flex flex-col items-center justify-between gap-3 border-t border-white/10 pt-8 text-xs text-slate-500 sm:flex-row"><p>© {new Date().getFullYear()} Shalean Cleaning Services. All rights reserved.</p><p>Built with ♥ for clean homes.</p></div>
        </PublicPageContainer>
      </footer>
      {showFloatingWhatsApp ? (
        <a href={waHref} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("whatsapp_click")} aria-label="Chat on WhatsApp" className={stackFloats ? "fixed bottom-[calc(5.25rem+2.75rem+0.5rem+env(safe-area-inset-bottom))] right-4 z-[9999] flex h-14 w-14 items-center justify-center rounded-full bg-green-500 text-white shadow-xl transition hover:scale-105 hover:bg-green-600 sm:right-6 md:bottom-[4.75rem]" : "fixed bottom-4 right-4 z-[9999] flex h-14 w-14 items-center justify-center rounded-full bg-green-500 text-white shadow-xl transition hover:scale-105 hover:bg-green-600 sm:bottom-6 sm:right-6"}><WhatsAppIcon className="h-7 w-7" /></a>
      ) : null}
    </>
  );
}
...
function FooterColumn({ title, links }: { title: string; links: ReadonlyArray<{ label: string; href: string }> }) {
  return <div><p className="text-xs font-semibold uppercase tracking-widest text-blue-300">{title}</p><ul className="mt-4 space-y-1 text-sm sm:space-y-2.5">{links.map(({ label, href }) => <li key={label}><Link href={href} className="block py-2 transition hover:text-white sm:py-0">{label}</Link></li>)}</ul></div>;
}
