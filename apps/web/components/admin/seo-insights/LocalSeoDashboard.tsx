...
type Payload = {
  summary:{areas:number;healthy:number;watch:number;action:number;gbpConnected:boolean;gbpHealth:string;gbpLastSync:string|null};
  rows:Row[];
};
...
export function LocalSeoDashboard(){
  ...
  async function save(){
    ...
    setSaving(false);
    if(!res.ok){
      setSaveError(res.error||"Could not save the local SEO area.");
      return;
    }
    setForm({area_name:"",location_path:"",review_count:"",average_rating:"",local_pack_position:"",local_pack_keyword:"",notes:""});
    await refetch();
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[["Areas",data?.summary.areas],["Healthy",data?.summary.healthy],["Watch",data?.summary.watch],["Action",data?.summary.action],["GBP",data?.summary.gbpConnected?`${data?.summary.gbpHealth||"unknown"}`:"Not connected"]].map(([k,v])=><div key={String(k)} className="rounded-2xl border bg-white p-4"><div className="text-xs font-semibold uppercase tracking-wide text-slate-500">{k}</div><div className="mt-2 text-2xl font-bold capitalize">{v??0}</div></div>)}</div>
