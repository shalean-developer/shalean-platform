...
function freshnessState(syncedAt:string|null|undefined):"healthy"|"stale"|"failed" {
  if (!syncedAt) return "failed";
  const ageHours = (Date.now() - new Date(syncedAt).getTime()) / 3_600_000;
  if (!Number.isFinite(ageHours) || ageHours > 72) return "failed";
...
export function SeoManagementCommandCentre() {
  ...
  const open = issues.data?.counts?.open ?? issueRows.filter(r=>r.workflow_status==="open").length;
  const inProgress = issues.data?.counts?.in_progress ?? issueRows.filter(r=>r.workflow_status==="in_progress").length;
  const applied = issues.data?.counts?.applied ?? issueRows.filter(r=>r.workflow_status==="applied").length;
  const verified = issues.data?.counts?.verified ?? issueRows.filter(r=>r.workflow_status==="verified").length;
  ...
  const priority = [
    ...issueRows.filter(r=>r.workflow_status==="open" && r.severity==="critical").slice(0,4).map(r=>({title:r.title,detail:r.slug?`/${r.slug}`:"SEO recommendation",href:"/office/seo-insights/issues"})),
    ...criticalPages.slice(0,4).map(r=>({title:r.title ?? r.path ?? r.slug ?? "SEO page issue",detail:r.issues[0]?.message ?? "Critical page health issue",href:r.path?.startsWith("/services")?"/office/seo-insights/service-health":"/office/seo-insights/blog-health"})),
  ].slice(0,6);

  const totalGroups = groups.data?.total_page_count ?? groups.data?.groups?.reduce((n,g)=>n+g.pages,0) ?? 0;
  const failedAutomation = (automation.data?.runs ?? []).some(run=>run.status==="error");
  ...
  return <section className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50/70 p-5 shadow-sm">
    ...
    <div className="grid gap-4 xl:grid-cols-3">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5"><h3 className="font-semibold text-slate-900">Automation health</h3><div className="mt-4 space-y-3"><div className={cn("rounded-xl border p-3 text-sm",tone(freshness))}><div className="flex items-center gap-2 font-semibold"><ShieldCheck className="h-4 w-4"/>GSC data: {freshness}</div><p className="mt-1 text-xs opacity-80">{groups.data?.synced_at ? `Last sync ${new Date(groups.data.synced_at).toLocaleString("en-ZA")}` : "No successful sync timestamp available."}</p></div><div className={cn("rounded-xl border p-3 text-sm",latestRun?.status==="error"?tone("failed"):"border-slate-200 bg-slate-50 text-slate-800")}><div className="flex items-center gap-2 font-semibold"><Clock3 className="h-4 w-4"/>Latest automation run</div><p className="mt-1 text-xs opacity-80">{latestRun ? `${latestRun.job} · ${latestRun.status} · ${new Date(latestRun.created_at).toLocaleString("en-ZA")}` : "No recent automation history loaded."}</p></div><Link href="/office/seo-insights/automation" className="inline-flex text-xs font-semibold text-slate-700 hover:underline">Open automation history →</Link></div></div>
    </div>
