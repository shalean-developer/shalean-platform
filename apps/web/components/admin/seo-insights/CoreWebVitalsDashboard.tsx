...
export function CoreWebVitalsDashboard(){
  const { data, loading, error, refetch } = useAdminData<Payload>("/api/admin/seo-insights/web-vitals");
  ...
  async function run(){
    setRunning(true);
    setRunError(null);
    const result = await adminFetch("/api/admin/seo-insights/web-vitals", { method:"POST" });
    if(!result.ok) setRunError(result.error || "Measurement failed.");
    await refetch();
    setRunning(false);
  }
  ...
  const visibleError = runError || error;
  return <div className="space-y-6">
    ...
    {visibleError&&<div className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{visibleError}</div>}
