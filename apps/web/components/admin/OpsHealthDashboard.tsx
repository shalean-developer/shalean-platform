...
const statusCopy: Record<HealthStatus, { label: string; description: string }> = {
  ...
  degraded: {
    label: "Degraded",
    description: "Active production issues detected.",
  },
  down: {
    label: "Down",
    description: "Critical production outage detected.",
  },
  critical: {
    label: "Critical",
...
function statusClass(status: HealthStatus): string {
  if (status === "critical" || status === "down") return "border-rose-300 bg-rose-50 text-rose-950 dark:border-rose-900/60 dark:bg-rose-950/30 dark:text-rose-100";
...
function diagnosticLines(diagnostics?: Record<string, unknown>): string[] {
  ...
  return Object.entries(diagnostics)
    .filter(([key]) => key !== "autoGenerated" && key !== "source")
    .slice(0, 6)
    .map(([key, value]) => `${key}: ${formatDiagnosticValue(value)}`);
}

function isAutoGeneratedFinding(finding: OpsHealthSummary): boolean {
  return finding.diagnostics?.autoGenerated === true || finding.code.startsWith("service_");
}
...
export function OpsHealthDashboard({
  ...
  const status = statusCopy[data.status] ?? statusCopy.degraded;
  const StatusIcon =
    data.status === "healthy" ? CheckCircle2 : data.status === "critical" || data.status === "down" ? ShieldAlert : AlertTriangle;
                ...
                >
                  ...
                  {onAcknowledge || onResolveAcknowledgement ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {finding.diagnostics?.serviceId && onViewService ? (
                        <button
                          type="button"
                          onClick={() => onViewService(String(finding.diagnostics?.serviceId))}
                      ...
                      {!isAutoGeneratedFinding(finding) && finding.diagnostics?.acknowledged === true && onResolveAcknowledgement ? (
                        <button
                          type="button"
                      ...
                      ) : !isAutoGeneratedFinding(finding) && onAcknowledge ? (
                        <button
                          type="button"
