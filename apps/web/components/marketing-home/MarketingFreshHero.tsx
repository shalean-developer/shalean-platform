...
/**
 * Bump `HERO_IMAGE_VERSION` whenever you replace hero assets under `public/images/marketing/`.
 * Next.js `/_next/image` and browsers cache by URL; a new query forces fresh assets.
 */
const HERO_IMAGE_VERSION = "20260429d";
const heroAsset = (filename: string) => `/images/marketing/${filename}?v=${HERO_IMAGE_VERSION}`;

const HERO_MAIN = heroAsset("cape-town-house-cleaning-kitchen.webp");
const HERO_TRUST = heroAsset("professional-cleaner-cape-town.webp");
          ...
          <div className="relative z-10 flex max-w-xl flex-col lg:max-w-none lg:pr-2">
            <h1 className="font-sans text-4xl font-extrabold leading-[1.06] tracking-tight text-gray-900 sm:text-5xl md:text-6xl lg:text-[3.35rem] lg:leading-[1.05] xl:text-7xl">
              <div className="overflow-hidden">
                <m.span custom={0} variants={fadeUp} initial="hidden" animate="visible" className="block">
                  A Clean Home
                </m.span>
              </div>
              <div className="overflow-hidden">
                <m.span custom={1} variants={fadeUp} initial="hidden" animate="visible" className="block">
                  A Fresh Start
                </m.span>
              </div>
            </h1>
