...
describe("SR-05B2C finance, billing and payout RBAC", () => {
  ...
  it("loads payment reconciliation with the payment.reconcile permission advertised by Office", async () => {
  ...
  it("requires payout.prepare before a dispute resolution can create an earnings adjustment", async () => {
    vi.mocked(requireAdminApi).mockResolvedValueOnce(OK_AUTH).mockResolvedValueOnce(DENIED_AUTH);
    const request = new Request(`https://example.test/api/admin/cleaner-earnings-disputes/${DISPUTE_ID}`, {
      method: "PATCH",
  ...
  it("keeps dispute resolution available without an earnings adjustment", async () => {
    vi.mocked(requireAdminApi).mockResolvedValue(OK_AUTH);
    const maybeSingle = vi.fn().mockResolvedValue({
      data: {
        id: DISPUTE_ID,
        cleaner_id: "00000000-0000-4000-8000-0000000000c1",
    ...
    const from = vi.fn((table: string) => {
      if (table === "cleaner_earnings_disputes") {
        return { select: vi.fn(() => ({ eq: vi.fn(() => ({ maybeSingle })) })), update };
      }
      return {};
    });
    ...
    const request = new Request(`https://example.test/api/admin/cleaner-earnings-disputes/${DISPUTE_ID}`, {
      method: "PATCH",
      headers: { Authorization: "Bearer test", "Content-Type": "application/json" },
      body: JSON.stringify({ status: "resolved", admin_response: "Resolved without adjustment." }),
    });
    const response = await disputePATCH(request, { params: Promise.resolve({ id: DISPUTE_ID }) });
    expect(response.status).toBe(200);
    expect(requireAdminApi).toHaveBeenCalledTimes(1);
    expect(requireAdminApi).toHaveBeenCalledWith(request);
  });

  it("hides adjustment controls unless the user has payout.prepare", () => {
    const pageSource = readFileSync(
      fileURLToPath(new URL("../../../(ui-redesign)/office/disputes/page.tsx", import.meta.url)),
      "utf8",
    );
    expect(pageSource).toContain('useAdminData<PermissionsPayload>("/api/admin/security/my-permissions")');
    expect(pageSource).toContain('permissionsData?.permissions?.includes("payout.prepare") === true');
    expect(pageSource).toContain('canPreparePayout && status === "resolved"');
