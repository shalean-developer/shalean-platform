...
describe("SR-05B2C finance, billing and payout RBAC", () => {
  it("blocks Paystack backfill when the caller lacks payment.reconcile", async () => {
    ...
    const request = new Request("https://example.test/api/admin/payments/backfill-paystack", { method: "POST", headers: { Authorization: "Bearer test" } });
  ...
  it("loads payment reconciliation with the same payment.reconcile permission advertised by Office", async () => {
    ...
    const request = new Request("https://example.test/api/admin/payment-reconciliation", { headers: { Authorization: "Bearer test" } });
  ...
  it("requires payout.prepare before a dispute resolution can create an earnings adjustment", async () => {
    ...
    const request = new Request(`https://example.test/api/admin/cleaner-earnings-disputes/${DISPUTE_ID}`, {
      ...
      body: JSON.stringify({ status: "resolved", admin_response: "Reviewed and resolved.", adjustment_amount_cents: 2500, adjustment_reason: "Correct underpayment" }),
    });
  ...
  it("keeps payout approval and release as separate explicit authorities", () => {
    const approveSource = readFileSync(fileURLToPath(new URL("../payouts/[id]/approve/route.ts", import.meta.url)), "utf8");
    const paySource = readFileSync(fileURLToPath(new URL("../payouts/[id]/pay/route.ts", import.meta.url)), "utf8");
    const markPaidSource = readFileSync(fileURLToPath(new URL("../payouts/[id]/mark-paid/route.ts", import.meta.url)), "utf8");
