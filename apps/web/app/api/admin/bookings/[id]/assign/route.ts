...
import { adminAssignCleanerToBooking } from "@/lib/booking/bookingOperations";
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const op = await adminAssignCleanerToBooking({
    admin,
    bookingId,
    cleanerId,
  ...
  const data = op.data;
  if (!data) {
    return NextResponse.json({ error: "Assign did not return a result." }, { status: 500 });
  }
