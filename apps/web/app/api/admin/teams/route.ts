...
export async function GET(request: Request) {
  ...
  if (supervisorScope.isSupervisor) {
    if (!supervisorScope.teamIds.length) {
      return NextResponse.json({ teams: [], supervisor_view: true, members: [], earnings: { month_total_cents: 0, pending_cents: 0, approved_cents: 0, paid_cents: 0 } });
    }
  ...
  if (ids.length > 0) {
    ...
    for (const r of memberRows ?? []) {
      ...
      if (tid) countByTeam.set(tid, (countByTeam.get(tid) ?? 0) + 1);
    }
  }
  ...
  if (!supervisorScope.isSupervisor) return NextResponse.json({ teams: teamsWithCounts, supervisor_view: false });

  const now = new Date();
  const monthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1)).toISOString();
  const totals = { month_total_cents: 0, pending_cents: 0, approved_cents: 0, paid_cents: 0 };
  const earningsByCleaner = new Map<string, { total_cents: number; pending_cents: number; approved_cents: number; paid_cents: number }>();
  ...
  if (supervisorScope.cleanerIds.length > 0) {
    ...
    for (const raw of earningRows ?? []) {
      ...
      const item = earningsByCleaner.get(cleanerId) ?? { total_cents: 0, pending_cents: 0, approved_cents: 0, paid_cents: 0 };
      ...
      else if (status === "approved") { item.approved_cents += amount; totals.approved_cents += amount; }
      else { item.pending_cents += amount; totals.pending_cents += amount; }
      earningsByCleaner.set(cleanerId, item);
    }
  }

  const { data: cleanerRows, error: cleanerErr } = supervisorScope.cleanerIds.length
    ? await admin.from("cleaners").select("id, full_name, status, is_available, jobs_completed, rating").in("id", supervisorScope.cleanerIds)
    : { data: [], error: null };
  if (cleanerErr) return NextResponse.json({ error: cleanerErr.message }, { status: 500 });
  const teamByCleaner = new Map<string, string>();
  if (ids.length > 0) {
    const { data: membershipRows, error: membershipErr } = await admin
      .from("team_members").select("team_id, cleaner_id").in("team_id", ids).is("active_to", null);
    if (membershipErr) return NextResponse.json({ error: membershipErr.message }, { status: 500 });
    for (const raw of membershipRows ?? []) {
      const row = raw as { team_id?: string | null; cleaner_id?: string | null };
      if (row.cleaner_id && row.team_id) teamByCleaner.set(row.cleaner_id, row.team_id);
    }
  }
  const members = (cleanerRows ?? []).map((raw) => {
    ...
    return {
      cleaner_id: row.id,
      team_id: teamByCleaner.get(row.id) ?? null,
      name: row.full_name ?? "Cleaner",
      status: row.status ?? null,
      is_available: row.is_available ?? null,
      jobs_completed: row.jobs_completed ?? 0,
      rating: row.rating ?? null,
      earnings: earningsByCleaner.get(row.id) ?? { total_cents: 0, pending_cents: 0, approved_cents: 0, paid_cents: 0 },
    };
  }).sort((a, b) => a.name.localeCompare(b.name));

  return NextResponse.json({ teams: teamsWithCounts, supervisor_view: true, members, earnings: totals, earnings_period_start: monthStart });
}
...
export async function POST(request: Request) {
  ...
  if (supervisorScope.isSupervisor) return NextResponse.json({ error: "Supervisors cannot create teams." }, { status: 403 });
...
export async function DELETE(request: Request) {
  ...
  if (!admin) return NextResponse.json({ error: "Server configuration error." }, { status: 503 });
  const supervisorScope = await resolveSupervisorTeamScope(admin, auth.userId);
  if (supervisorScope.isSupervisor) return NextResponse.json({ error: "Supervisors cannot delete teams." }, { status: 403 });
  let body: { teamId?: string };
  try { body = (await request.json()) as { teamId?: string }; } catch { return NextResponse.json({ error: "Invalid JSON." }, { status: 400 }); }
  const teamId = String(body.teamId ?? "").trim();
  if (!teamId) return NextResponse.json({ error: "teamId required." }, { status: 400 });
  const { count, error: countErr } = await admin.from("bookings").select("id", { count: "exact", head: true }).eq("team_id", teamId).eq("is_team_job", true).in("status", ["pending", "assigned", "in_progress"]);
  if (countErr) return NextResponse.json({ error: countErr.message }, { status: 500 });
  if ((count ?? 0) > 0) return NextResponse.json({ error: "Cannot delete team with active bookings." }, { status: 409 });
