...
import { requireAnyAdminPermissionFromRequest, requireAdminPermissionFromRequest } from "@/lib/admin/requirePermission";
...
function configuredAdminClient(): SupabaseClient<any, any, any> | null {
...
async function loadUsers(admin: SupabaseClient<any, any, any>): Promise<Map<string, string>> {
  ...
  for (;;) {
    ...
    if (data.users.length < 1000 || page >= 10) break;
    page += 1;
  }
  return users;
}
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("Admin access review user lookup failed", { message: error instanceof Error ? error.message : "unknown" });
    return NextResponse.json({ error: "Unable to load access review users." }, { status: 500 });
  }

  const [assignmentsRes, rolesRes, reviewsRes] = await Promise.all([
...
export async function POST(request: Request) {
  ...
  if (auditError) {
    ...
    const rollback = await admin.from("admin_access_reviews").delete().eq("id", review.id);
    if (rollback.error) console.error("Admin access review rollback failed", { reviewId: review.id, code: rollback.error.code });
