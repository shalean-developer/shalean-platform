...
export async function GET(request: Request) {
  ...
  let query = admin
    .from("bookings")
    .select(
      "id, date, city_id, cleaner_id, service_id, total_paid_zar, amount_paid_cents, total_paid_cents, service_fee_cents, company_revenue_cents, earnings_summary, cleaner_payout_cents, display_earnings_cents, cleaner_earnings_total_cents, cleaner_bonus_cents, cities ( name ), pricing_services ( name )",
      { count: "exact" },
    )
    .eq("status", "completed")
    .eq("is_test", false)
  ...
  const items = await Promise.all(
    (bookings ?? []).map(async (b) => {
      ...
      const service = b.pricing_services as { name?: string } | null;
      return {
        booking_id: b.id,
        date: b.date,
