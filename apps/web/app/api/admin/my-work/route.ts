...
async function marketingItems(permissions: ReadonlySet<string>): Promise<OfficeWorkItem[]> {
  ...
  if (permissions.has("content.publish")) {
    const { data: publishablePromotions } = await admin.from("promotions").select("id").in("status", ["active", "scheduled"]).limit(2000);
    const promotionIds = (publishablePromotions ?? []).map((promotion) => promotion.id).filter(Boolean);
    if (promotionIds.length) {
      const { data } = await admin.from("campaign_content").select("id,title,channel,status,created_at,updated_at,promotion_id").eq("status", "ready").in("promotion_id", promotionIds).order("updated_at", { ascending: true }).limit(1000);
      for (const c of data ?? []) { const touched = c.updated_at ?? c.created_at; const ms = touched ? Date.parse(touched) : Number.NaN; const overdue = Number.isFinite(ms) && now - ms >= 3 * 86400000; items.push({ id: `marketing.campaign_ready:${c.id}`, type: "marketing.campaign_ready", title: `${cleanLabel(c.channel) || "Campaign"} content ready for publishing review`, summary: c.title?.trim() || "Campaign content is ready for review.", priority: overdue ? "high" : "medium", status: overdue ? "overdue" : "open", href: `/office/marketing?content=${encodeURIComponent(c.id)}`, actionLabel: "Review campaign", requiredPermission: "content.publish", occurredAt: touched, dueAt: Number.isFinite(ms) ? new Date(ms + 3 * 86400000).toISOString() : null, branchId: null, teamId: null }); }
    }
  }
  return items;
}
