...
async function customerCareItems(permissions: ReadonlySet<string>, branchIds: readonly string[]): Promise<OfficeWorkItem[]> {
  ...
  return pending.flatMap(([phone, e]) => { const branchId = cityByPhone.get(phone) ?? null; if (!global && (!branchId || !allowed.has(branchId))) return []; const occurred = e.created_at ? Date.parse(e.created_at) : Number.NaN; const wait = Number.isFinite(occurred) ? Math.max(0, now - occurred) : 0; const critical = wait >= 4 * 3600000; const overdue = wait >= 3600000; return [{ id: `customer_care.whatsapp_reply:${e.id}`, type: "customer_care.whatsapp_reply", title: critical ? `WhatsApp customer ${maskedPhone(phone)} has waited over 4 hours` : `WhatsApp customer ${maskedPhone(phone)} needs a reply`, summary: e.created_at ? `Latest inbound message received ${e.created_at}` : "Latest inbound WhatsApp message has no admin reply.", priority: critical ? "critical" : overdue ? "high" : "medium", status: overdue ? "overdue" : "open", href: `/office/notifications?conversation=${encodeURIComponent(phone)}`, actionLabel: "Reply on WhatsApp", requiredPermission: "customer.contact", occurredAt: e.created_at, dueAt: Number.isFinite(occurred) ? new Date(occurred + 3600000).toISOString() : null, branchId, teamId: null } satisfies OfficeWorkItem]; });
}
