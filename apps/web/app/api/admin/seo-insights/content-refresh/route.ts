...
type RefreshRow = {
  ...
  completed_clicks:number|null; completed_impressions:number|null; completed_position:number|null;
  verified_clicks:number|null; verified_impressions:number|null; verified_position:number|null;
  baseline_captured_at:string|null; completed_at:string|null; verification_due_at:string|null; verified_at:string|null;
};
...
export async function GET(request:Request){
  ...
  const latestByPost=new Map<string,RefreshRow>(); for(const r of (refreshes??[]) as RefreshRow[]){ if(!latestByPost.has(r.blog_post_id)&&r.status!=="cancelled") latestByPost.set(r.blog_post_id,r); }
  const queue=((posts??[]) as BlogPost[]).flatMap(post=>{
    const gsc=metricsBySlug.get(post.slug)??null; const reasons=declined(gsc); const refresh=latestByPost.get(post.id)??null;
    if(!reasons.length&&!refresh) return [];
    const comparison=refresh&&refresh.status==="completed"?(()=>{
      const clicks=refresh.verified_at?refresh.verified_clicks:gsc?.clicks;
      const impressions=refresh.verified_at?refresh.verified_impressions:gsc?.impressions;
      const position=refresh.verified_at?refresh.verified_position:gsc?.avg_position;
      if(clicks==null||impressions==null) return null;
      return {clicks_delta:clicks-(refresh.baseline_clicks??0),impressions_delta:impressions-(refresh.baseline_impressions??0),position_delta:refresh.baseline_position==null||position==null?null:refresh.baseline_position-position};
    })():null;
...
export async function POST(request:Request){
  ...
  const {error:historyError}=await admin.from("seo_content_refresh_history").insert({refresh_id:refresh.id,action:"queued",actor_email:auth.email??null,details:{due_date:refresh.due_date,owner_email:refresh.owner_email,editor_email:refresh.editor_email,reason_codes:refresh.reason_codes}});
  if(historyError) return NextResponse.json({error:`Refresh queued but history write failed: ${historyError.message}`},{status:500});
...
export async function PATCH(request:Request){
  ...
  const slug=existing.blog_posts?.slug;
  if(body.status==="completed"&&existing.status!=="completed"){
    const {data:gsc}=await admin.from("site_gsc_metrics").select("page_url,clicks,impressions,avg_position").eq("page_group","blog").limit(1000); const metric=(gsc??[]).find((r:any)=>slugFromUrl(r.page_url)===slug)??null;
  ...
  if(body.verify===true){
    const {data:gsc}=await admin.from("site_gsc_metrics").select("page_url,clicks,impressions,avg_position").eq("page_group","blog").limit(1000); const metric=(gsc??[]).find((r:any)=>slugFromUrl(r.page_url)===slug)??null;
    update.verified_at=new Date().toISOString(); update.verified_clicks=metric?.clicks??null; update.verified_impressions=metric?.impressions??null; update.verified_position=metric?.avg_position??null;
  }
  ...
  const {error:historyError}=await admin.from("seo_content_refresh_history").insert({refresh_id:id,action:body.verify===true?"verified":String(body.status??"updated"),actor_email:auth.email??null,details:update});
  if(historyError) return NextResponse.json({error:`Refresh updated but history write failed: ${historyError.message}`},{status:500});
