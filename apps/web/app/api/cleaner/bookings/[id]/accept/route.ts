...
import { cleanerAcceptBooking } from "@/lib/booking/bookingOperations";
...
/** REST alias for `POST /api/cleaner/jobs/:id` with `{ "action": "accept" }` (plus idempotency on the jobs route). */
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const op = await cleanerAcceptBooking({
    admin,
