...
export async function GET(_request: Request, ctx: { params: Promise<{ token: string }> }) {
  ...
  return NextResponse.json({
    ...
    request: {
      ...
      requestedAt: loaded.request.requested_at,
      approvedAt: loaded.request.approved_at,
      approvalSource: loaded.request.approval_source,
      booking: loaded.request.bookings,
    },
  });
}
...
export async function POST(request: Request, ctx: { params: Promise<{ token: string }> }) {
  ...
  if (body?.decision !== "approve" && body?.decision !== "reject") {
    return NextResponse.json({ error: "Choose whether the cleaning is complete." }, { status: 400 });
  }
  ...
  if (["customer_approved", "customer_rejected", "admin_approved"].includes(String(loaded.request.status))) {
    return NextResponse.json({ ok: true, status: loaded.request.status, duplicate: true });
  }
  if (loaded.request.status !== "pending") {
    return NextResponse.json({ error: "This early-finish request is no longer awaiting a customer response." }, { status: 409 });
  }
  ...
  const patch = approved
    ? {
        ...
        approved_at: now,
        approved_by: "customer_confirmation_link",
  ...
  const { data: updated, error } = await loaded.admin
    .from("cleaner_early_finish_requests")
    .update(patch)
