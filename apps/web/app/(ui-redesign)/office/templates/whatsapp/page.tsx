...
export default function WhatsAppTemplateReadinessPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return (data?.templates ?? []).filter((item) => {
      const matchesSearch =
        ...
        item.metaTemplateName.toLowerCase().includes(q) ||
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 max-w-3xl text-sm text-slate-500">
            Meta approval and mapping status for proactive WhatsApp messages. Only approved templates are considered ready to send outside the 24-hour conversation window.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs leading-5 text-slate-600">
              A Supabase template being active does not mean Meta approved it. Unknown, pending, and rejected templates remain not ready until their Meta approval state is explicitly recorded.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-3 border-b border-slate-100 p-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search Shalean key or Meta template name…"
              ...
              {(["all", "customer", "cleaner"] as const).map((value) => (
                <button
                  ...
                  className={cn(
                    ...
                    audience === value ? "bg-white text-slate-900 shadow-sm" : "text-slate-500",
                  )}
                >
                  {value}
                </button>
              ...
              {(["all", "approved", "pending", "unknown", "rejected"] as const).map((value) => (
                <button
                  ...
                  className={cn(
                    ...
                    status === value ? "bg-white text-slate-900 shadow-sm" : "text-slate-500",
                  )}
                >
                ...
                <div key={item.key} className="grid gap-3 px-5 py-4 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1.4fr)_auto_auto] lg:items-center">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="truncate font-mono text-sm font-semibold text-slate-800">{item.key}</p>
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-slate-400">
                      Mapping: {item.mappingSource === "env" ? "Vercel override" : "default Shalean key"}
                    </p>
                  </div>
                  <span className={cn("inline-flex w-fit items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold", statusConfig.className)}>
                  ...
                  <span
                    className={cn(
                      ...
                      item.sendReady ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500",
                    )}
                  >
                    {item.sendReady ? "Ready to send" : "Not ready"}
                  </span>
                </div>
              );
            })}
      ...
      {data?.fetchedAt ? (
        <p className="text-xs text-slate-400">Status refreshed {new Date(data.fetchedAt).toLocaleString()}.</p>
      ) : null}
    </div>
  );
}
