    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          ...
          <p className="mb-4 text-xs text-slate-500">Sessions viewing each booking step</p>
          {loading && !data ? (
        ...
        <p className="mt-0.5 text-sm text-slate-500">
          ...
          {windowLabel ? (
            <span className="ml-1 text-slate-400">
              Since {windowLabel} · {sessions.toLocaleString("en-ZA")} sessions · {eventRows.toLocaleString("en-ZA")} events
            </span>
          ) : null}
        </p>
      </div>
...
function DailyTrendChart({ rows, max }: { rows: FunnelDailyTrendRow[]; max: number }) {
  return (
    <div className="flex h-44 items-end gap-1.5 overflow-x-auto pb-1">
      {rows.map((d) => (
        <div key={d.date} className="flex min-w-[28px] flex-1 flex-col items-center justify-end gap-0.5" title={`${d.date}: ${d.starts} starts, ${d.completed} completed`}>
          <div className="flex w-full max-w-[14px] flex-col items-center justify-end gap-0.5">
            <div className="w-full rounded-t bg-rose-300" style={{ height: `${Math.max(2, (d.paystackAbandons / max) * 120)}px` }} />
            <div className="w-full rounded-t bg-emerald-500" style={{ height: `${Math.max(2, (d.completed / max) * 120)}px` }} />
            <div className="w-full rounded-t bg-blue-500" style={{ height: `${Math.max(2, (d.starts / max) * 120)}px` }} />
          </div>
...
function StepConversionPanel({ rows, loading }: { rows: FunnelStepConversionRow[]; loading: boolean }) {
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      ...
      <p className="mb-4 text-xs text-slate-500">Share of sessions progressing between product steps</p>
      {loading ? (
