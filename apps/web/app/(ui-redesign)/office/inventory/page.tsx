    ...
    <div className="space-y-6 p-4 md:p-6">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <form className="space-y-3 rounded-xl border border-slate-200 bg-white p-4" onSubmit={(event) => { event.preventDefault(); void send("/api/admin/inventory/actions", { action: movement.movement_type === "issue" ? "issue" : "movement", ...movement, quantity: Number(movement.quantity) }); }}>
          ...
          <div className="grid grid-cols-2 gap-2">
            <select required value={movement.item_id} onChange={(e) => { const next = e.target.value; const selected = items.find((item) => item.id === next); const movementType = selected?.item_type === "equipment" && movement.movement_type === "consume" ? "issue" : selected?.item_type === "supply" && movement.movement_type === "issue" ? "consume" : movement.movement_type; setMovement({ ...movement, item_id: next, movement_type: movementType }); }} className="rounded-md border p-2 text-sm"><option value="">Select item</option>{items.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.sku} · {item.name}</option>)}</select>
            <select value={movement.movement_type} onChange={(e) => setMovement({ ...movement, movement_type: e.target.value })} className="rounded-md border p-2 text-sm"><option value="purchase">Purchase / receive</option>{items.find((item) => item.id === movement.item_id)?.item_type !== "equipment" ? <option value="consume">Consume on booking</option> : <option value="issue">Issue equipment</option>}<option value="loss">Record loss</option><option value="adjustment_in">Adjustment in</option><option value="adjustment_out">Adjustment out</option></select>
