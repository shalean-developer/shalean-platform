...
function friendlyVariableName(value: string) {
  return value.replace(/_/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase());
}
function formatJohannesburg(value: string) {
...
export function WhatsAppInbox() {
  ...
  useEffect(() => {
    ...
    for (const variable of chosenTemplate.variables) {
      if (variable === "customer_name") next[variable] = contact?.name?.trim() || "";
      else if (variable === "booking_id") next[variable] = contact?.bookingReference?.trim() || contact?.bookingId?.trim() || "";
      else next[variable] = "";
    }
    setTemplateValues(next);
    setSendError(null);
  }, [chosenTemplate?.key, selectedPhone, contact?.name, contact?.bookingId, contact?.bookingReference]);

  const templateValuesArray = chosenTemplate?.variables.map((variable) => (templateValues[variable] ?? "").trim()) ?? [];
  const missingTemplateVariables = chosenTemplate?.variables.filter((variable) => !(templateValues[variable] ?? "").trim()) ?? [];
  const templateReadyToSend = Boolean(chosenTemplate && missingTemplateVariables.length === 0);
  const templatePreview = chosenTemplate
    ? chosenTemplate.body.replace(/\{\{(\d+)\}\}/g, (_match, n: string) => {
        const variable = chosenTemplate.variables[Number(n) - 1];
        return variable ? (templateValues[variable] ?? "").trim() || `{{${n}}}` : `{{${n}}}`;
      })
    : "";
  ...
  async function sendReply() {
    ...
    if (!isText && (!chosenTemplate || !templateReadyToSend)) {
      setSending(false);
      setSendError(missingTemplateVariables.length
        ? `Complete: ${missingTemplateVariables.map(friendlyVariableName).join(", ")}.`
        : "Select an approved customer template.");
      return;
    }
    const result = await adminFetch<{ ok: boolean; messageId?: string | null; adminEmail?: string | null; createdAt?: string }>("/api/admin/whatsapp-inbox/reply", {
      ...
      body: JSON.stringify(isText
        ...
        : { phone: selectedPhone, mode: "template", templateName, language: chosenTemplate?.language ?? "en", bodyParams: templateValuesArray }),
    });
    setSending(false);
    if (!result.ok) {
    ...
    const sentBody = isText ? textBody : templatePreview || "[Template message]";
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <span className="shrink-0 text-[10px] text-slate-400">{new Date(latest.createdAt).toLocaleTimeString("en-ZA", { timeZone: "Africa/Johannesburg", hour: "2-digit", minute: "2-digit" })}</span>
                      </div>
                  ...
                  <div key={message.id} className={`flex ${message.direction === "outbound" ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[78%] rounded-2xl px-3.5 py-2.5 shadow-sm ${message.direction === "outbound" ? "bg-emerald-600 text-white" : "border border-slate-100 bg-white text-slate-800"}`}>
                      ...
                      <div className={`mt-1 flex items-center justify-end gap-2 text-[10px] ${message.direction === "outbound" ? "text-emerald-100" : "text-slate-400"}`}>
                        {message.direction === "outbound" ? <span>{message.adminEmail ? `Sent by ${message.adminEmail}` : "Sent by Shalean"}</span> : null}<span>{formatJohannesburg(message.createdAt)}</span>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                {state?.conversationOpen ? (
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <select value={templateName} onChange={(e) => setTemplateName(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-emerald-300">
                    ...
                    {chosenTemplate ? (
                      <>
                        <div className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5">
                          <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Message preview</p>
                          <p className="mt-1 whitespace-pre-wrap text-sm leading-5 text-slate-700">{templatePreview}</p>
                        </div>

                        {chosenTemplate.variables.length > 0 ? (
                          <div className="grid gap-3 sm:grid-cols-2">
                            {chosenTemplate.variables.map((variable, index) => (
                              <div key={variable}>
                                <label className="text-xs font-semibold text-slate-600">
                                  {friendlyVariableName(variable)} <span className="font-normal text-slate-400">{`{{${index + 1}}}`}</span>
                                </label>
                                <input
                                  value={templateValues[variable] ?? ""}
                                  onChange={(e) => {
                                    setTemplateValues((current) => ({ ...current, [variable]: e.target.value }));
                                    setSendError(null);
                                  }}
                                  placeholder={`Enter ${friendlyVariableName(variable).toLowerCase()}`}
                                  className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none focus:border-emerald-300"
                                />
                              </div>
                            ))}
                          </div>
                        ...
                        {missingTemplateVariables.length > 0 ? (
                          <p className="text-xs text-amber-700">Complete {missingTemplateVariables.map(friendlyVariableName).join(", ")} before sending.</p>
                        ) : (
                          <p className="text-xs text-emerald-700">All required template details are complete.</p>
                        )}
                      </>
                    ) : null}

                    <button type="button" onClick={() => void sendReply()} disabled={sending || !templateReadyToSend} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"><Send className="h-4 w-4" /> {sending ? "Sending…" : "Send approved template"}</button>
                  </div>
                )}
                {sendError ? <p className="mt-2 text-sm text-red-600">{sendError}</p> : null}
                <p className="mt-2 text-[11px] text-slate-400">Free-text replies are only allowed within 24 hours of the customer’s latest inbound WhatsApp message. Outside that window, Shalean requires an approved Meta template. Template fields are generated from the approved Meta template definition, and known customer or booking details are filled automatically when available.</p>
              </div>
            </>
          )}
        </section>
