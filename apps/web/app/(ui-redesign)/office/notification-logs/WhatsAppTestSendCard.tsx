...
import { AlertCircle, CheckCircle2, ChevronDown, ChevronUp, MessageSquare, Send } from "lucide-react";
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-4">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-base font-bold text-slate-900">WhatsApp Test</h2>
            <p className="truncate text-sm text-slate-500">
              {expanded
                ? "Test one approved Meta template at a time before relying on it in production."
                : `${approvedTemplates.length} approved templates available · Open only when you need to test.`}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {expanded ? <span className="hidden rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 sm:inline-flex">Uses WHATSAPP_PROVIDER</span> : null}
          <button
            type="button"
            onClick={() => setExpanded((value) => !value)}
            aria-expanded={expanded}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 text-sm font-bold text-blue-700 transition-colors hover:bg-blue-100"
          >
            {expanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
            {expanded ? "Hide tester" : "Open tester"}
          </button>
        </div>
      </div>
