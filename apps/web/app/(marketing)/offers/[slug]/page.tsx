...
export default async function OfferLandingPage({ params }: Props) {
  ...
  if (!promo || !allowedStatuses.has(String(promo.status ?? ""))) {
    // Prefer a soft landing over 404 for shared campaign/offer URLs.
      ...
      >
        <PublicPageContainer size="content" className="flex flex-col gap-6 py-16 md:py-24">
          ...
          <p className="max-w-xl text-lg text-white/90">
            {structured.heroSubheadline ?? promo.display_config.subheadline ?? promo.description}
          </p>
          {promo.promo_code ? (
      ...
      <PublicPageContainer size="content" className="space-y-12 py-12">
        <section>
          ...
          <ul className="mt-4 grid gap-3 sm:grid-cols-2">
            {(structured.benefits ?? [
              ...
              <li
                key={benefit}
                className="rounded-[var(--ui-radius-2xl)] border border-border bg-card p-4 text-sm text-card-foreground"
              >
                {benefit}
              </li>
            ))}
        ...
        <section>
          ...
          <ul className="mt-4 list-disc space-y-1 pl-5 text-muted-foreground">
            {(structured.servicesIncluded ?? ["Standard home cleaning"]).map((service) => (
        ...
        {(structured.faqs?.length ?? 0) > 0 ? (
          <section>
            ...
            <div className="mt-4 space-y-3">
              {(structured.faqs ?? []).map((faq) => (
                <details
                  key={faq.q}
                  className="rounded-[var(--ui-radius-2xl)] border border-border bg-card p-4 text-card-foreground"
                >
