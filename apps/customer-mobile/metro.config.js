...
/**
 * Customer mobile sits under apps/customer-mobile with file: links to packages/*.
 * Root package.json has no workspaces field, so Expo cannot auto-detect the
 * monorepo — watch packages/ only and prefer the app's node_modules.
 *
...
// Keep hierarchical lookup enabled: React Native nests packages such as
// `@react-native/virtualized-lists` under `react-native/node_modules` (npm file:
// monorepo without hoisting). `disableHierarchicalLookup: true` breaks Android.
