...
import {
  ...
  isValidContactPhone,
} from "@shalean/validation";
import { BOOKING_MIN_LEAD_MINUTES } from "@shalean/utils";
import { SERVICE_SLUGS } from "@/lib/booking/serviceMeta";
import {
  ...
  return (data, ctx) => {
    if (!isCustomerOnlineBookingTimeSlot(data.time, scheduling)) {
      ctx.addIssue({
        ...
        message: "Select a valid morning time (8:00 AM – 12:30 PM)",
        path: ["time"],
      });
    } else if (
      data.date &&
      !filterCustomerOnlineBookingTimeSlots(data.date, {
        scheduling,
        leadMinutes: scheduling?.leadMinutes ?? BOOKING_MIN_LEAD_MINUTES,
      }).includes(data.time.trim().slice(0, 5))
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (
      ...
      data.recurringFrequency === "custom" &&
